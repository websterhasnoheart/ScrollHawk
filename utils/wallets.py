...
from core_func.starkNet.basic_func import check_wallet_balance
...
def display_data(data):
    ...
    table.field_names = ["Index", "Wallet_Name", "Wallet_Address",
                         "L1 Balance", "zkEra Balance", "starkWallet", "starkNet Balance"]
    # Add rows to the table.
    for row in data:
        table.add_row([row["index"], row["name"], row["address"],
                       get_eth_balance(w3, row["address"]), check_balance(
                           row["privatekey"]),
                       row['argentWallet'][0:6] + "..." + row['argentWallet'][-4:-1], check_wallet_balance(row['argentWallet'])])
    print(table)
